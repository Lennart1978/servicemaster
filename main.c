...
void setup_dbus(sd_bus *bus)
{
    ...
    if (rc < 0)
        FAIL("Cannot subcribe to systemd dbus events: %s\n", strerror(-rc));
    ...
    if (rc < 0) 
        FAIL("Cannot register interest in daemon reloads: %s\n", strerror(-rc));

    rc = sd_event_default(&ev);
    if (rc < 0)
        FAIL("Cannot fetch default event handler: %s\n", strerror(-rc));

    rc = sd_bus_attach_event(bus, ev, SD_EVENT_PRIORITY_NORMAL);
    if (rc < 0)
        FAIL("Unable to attach bus to event loop: %s\n", strerror(-rc));
